...
async def init_broadcast_permissions(db: Database):
...
async def grant_permissions_to_user(db: Database, user_id: int, permissions: list, granted_by: int = None):
...
async def revoke_permissions_from_user(db: Database, user_id: int, permissions: list):
...
async def get_user_permissions(db: Database, user_id: int) -> list:
